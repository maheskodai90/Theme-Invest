 ...
 * @return array
 */
function northarc_capital_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'northarc_capital_page_menu_args' );
 ...
 * @return array
 */
function northarc_capital_body_classes( $classes ) {
...
add_filter( 'body_class', 'northarc_capital_body_classes' );


/**
 * Mark Posts/Pages as Untiled when no title is used
 */
add_filter( 'the_title', 'northarc_capital_title' );

function northarc_capital_title( $title ) {
  if ( $title == '' ) {
    return 'Untitled';
  } else {
    return $title;
...
function custom_password_form() {
  ...
  <div class="row">
    <div class="col-lg-10">
        <p>' . __( "This post is password protected. To view it please enter your password below:" ,'northarc_capital') . '</p>
        <label for="' . $label . '">' . __( "Password:" ,'northarc_capital') . ' </label>
      <div class="input-group">
        <input class="form-control" value="' . get_search_query() . '" name="post_password" id="' . $label . '" type="password">
        <span class="input-group-btn"><button type="submit" class="btn btn-default" name="submit" id="searchsubmit" vvalue="' . esc_attr__( "Submit",'northarc_capital' ) . '">' . __( "Submit" ,'northarc_capital') . '</button>
        </span>
      </div>
    </div>
  </div>
 ...
 */
add_filter( 'the_content', 'northarc_capital_add_custom_table_class' );
function northarc_capital_add_custom_table_class( $content ) {
  return str_replace( '<table>', '<table class="table table-hover">', $content );
}

if ( ! function_exists( 'northarc_capital_social_icons' ) ) :
/**
 * Display social links in footer and widgets
 */
function northarc_capital_social_icons(){
...
if( !function_exists( 'northarc_capital_social' ) ) :
/**
 * Fallback function for the deprecated function northarc_capital_social
 */
function northarc_capital_social(){
  if( of_get_option('footer_social') ) {
    northarc_capital_social_icons();
  }
}
endif;

if( !function_exists( 'northarc_capital_header_menu' ) ) :
/**
 * header menu (should you choose to use one)
 */
function northarc_capital_header_menu() {
 ...
 */
function northarc_capital_footer_links() {
  // display the WordPress Custom Menu if available
  wp_nav_menu(array(
    'container'       => '',                              // remove nav container
    'container_class' => 'footer-links ',   // class of container (should you choose to use it)
    'menu'            => __( 'Footer Links', 'northarc_capital' ),   // nav name
    'menu_class'      => 'lastmenu ',      // adding custom nav class
    ...
    'fallback_cb'     => 'northarc_capital_footer_links_fallback'  // fallback function
  ));
} /* end northarc_capital footer link */


/**
 * Get Post Views - for Popular Posts widget
 */
function northarc_capital_getPostViews($postID){
...
function northarc_capital_setPostViews($postID) {
    $count_key = 'post_views_count';
...
if ( ! function_exists( 'northarc_capital_call_for_action' ) ) :
/**
 * Call for action button & text area
 */
function northarc_capital_call_for_action() {
...
if ( ! function_exists( 'northarc_capital_featured_slider' ) ) :
/**
 * Featured image slider
 */
function northarc_capital_featured_slider() {
    if ( is_front_page() && of_get_option('northarc_capital_slider_checkbox') == 1 ) {
      echo '<div class="flexslider">';
        echo '<ul class="slides">';

          $count = of_get_option('northarc_capital_slide_number');
          $slidecat = of_get_option('northarc_capital_slide_categories');
...
if ( ! function_exists( 'northarc_capital_footer_info' ) ) :
/**
 * function to show the footer info, copyright information
 */
function northarc_capital_footer_info() {
  global $northarc_capital_footer_info;
  printf( __( 'Theme by %1$s Powered by %2$s', 'northarc_capital' ) , '<a href="http://colorlib.com/wp/" target="_blank">Colorlib</a>', '<a href="http://wordpress.org/" target="_blank">WordPress</a>');
}
endif;
 ...
 */
if (!function_exists('get_northarc_capital_theme_options'))  {
  function get_northarc_capital_theme_options(){
...
add_action('wp_head','get_northarc_capital_theme_options',10);
